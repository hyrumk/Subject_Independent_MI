...
if __name__ == '__main__':
    ...
    '''
    # loading and preprocessing all 9 subjects
    ...
    for i in range(9):
        ...
        # Where you use the data to make a filtered data
        X, Y, frequency_order = generate_ss_feature(data, binary = True)  # train data

        with open('data_train_X{}_binary.pkl'.format(CHOSEN_TEST_SUBJECT), 'wb') as f:
            pickle.dump(X, f)
        with open('data_train_Y{}_binary.pkl'.format(CHOSEN_TEST_SUBJECT), 'wb') as f:
            pickle.dump(Y, f)
        with open('data_freq_order{}_binary.pkl'.format(CHOSEN_TEST_SUBJECT), 'wb') as f:
            pickle.dump(frequency_order, f)

        X_test, Y_test = generate_ss_feature_test(data_test, frequency_order[:20], binary = True)  # test data

        with open('data_test_X{}_binary.pkl'.format(CHOSEN_TEST_SUBJECT), 'wb') as f:
            pickle.dump(X_test, f)
        with open('data_test_Y{}_binary.pkl'.format(CHOSEN_TEST_SUBJECT), 'wb') as f:
            pickle.dump(Y_test, f)
    ...
    for subject in range(9):
        CHOSEN_TEST_SUBJECT = subject + 1
        with open('data_train_X{}_binary.pkl'.format(CHOSEN_TEST_SUBJECT), 'rb') as f:
            X = pickle.load(f)
        with open('data_train_Y{}_binary.pkl'.format(CHOSEN_TEST_SUBJECT), 'rb') as f:
            Y = pickle.load(f)
        with open('data_freq_order{}_binary.pkl'.format(CHOSEN_TEST_SUBJECT), 'rb') as f:
            frequency_order = pickle.load(f)


        with open('data_test_X{}_binary.pkl'.format(CHOSEN_TEST_SUBJECT),'rb') as f:
            X_test = pickle.load(f)
        with open('data_test_Y{}_binary.pkl'.format(CHOSEN_TEST_SUBJECT),'rb') as f:
        ...
        result_string = sn.train_and_test(trainloader, 150, testloader, num_classes=2)
